...
def load_dataset(path: str) -> pd.DataFrame:
 ...
 df["reply"] = df["reply"].apply(lambda s: f"{START_TOKEN} " +
clean_text(s) + f" {END_TOKEN}")
 return df
def vectorize(df: pd.DataFrame, max_vocab: int = 12000, max_len: int =
32):
...
def build_train_model(vocab_size: int, emb_dim: int, hid_dim: int,
max_len: int):
 # Shared embedding
 enc_inputs = Input(shape=(max_len,), name="encoder_inputs")
 dec_inputs = Input(shape=(max_len,), name="decoder_inputs")
 emb = Embedding(vocab_size, emb_dim, mask_zero=True,
name="shared_embedding")
 ...
 dec_gru = GRU(hid_dim, return_sequences=True, return_state=True,
name="decoder_gru")
...
def save_artifacts(model, tokenizer, model_dir: str):
 ...
 # Save tokenizer
 with open(os.path.join(model_dir, "tokenizer.json"), "w", encoding="utf8") as f:f.write(tokenizer.to_json())
def main():
 parser = argparse.ArgumentParser()
 parser.add_argument('--data_path', type=str,
default='sample_pairs.csv')
 ...
 enc_in, dec_in, dec_tar, tokenizer, vocab_size = vectorize(df,
args.max_vocab, args.max_len)
 model = build_train_model(vocab_size, args.emb_dim, args.hid_dim,
args.max_len)
 history = model.fit(
 [enc_in, dec_in],
 dec_tar[..., None], # sparse targets
 epochs=args.epochs,
 ...
 with open(os.path.join(args.model_dir, 'meta.json'), 'w') as f: json.dump(meta, f, indent=2)
